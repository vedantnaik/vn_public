...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            for eachPos in self.legalPositions:
                    self.particles.append(eachPos)
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None: # No noise implies ghost in jail
            ...
            for eachPos in self.legalPositions:
                distance = util.manhattanDistance(eachPos, pacmanPosition)
                allPossible[eachPos] += emissionModel[distance] * oldBelief[eachPos]

            if (allPossible.totalCount() == 0): # if all particles receive zero weight
                self.initializeUniformly(gameState) # initialize the game state
            else:
                # else, sample uniformly using  util.sample(Counter object)
                temp = []
                count = 0
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        probability = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(probability)

        self.particles = []
        particleIndex = 0
        while particleIndex < self.numParticles:
            for j in probability:
                if particleIndex < self.numParticles:
    ...
    def observeState(self, gameState):
        ...
        legal = util.Counter()
        for index, eachParticle in enumerate(self.particles):
            partial = 1.0
            for ghostIndex in range(self.numGhosts):
                # no noise implies ghost in jail position
                if noisyDistances[ghostIndex] == None:
                    eachParticle = self.getParticleWithGhostInJail(eachParticle, ghostIndex)
                else:
                    # capture the distance of the ghost
                    distance = util.manhattanDistance(eachParticle[ghostIndex], pacmanPosition)
                    partial *= emissionModels[ghostIndex][distance]
            legal[eachParticle] += partial
        ...
        if not any(legal.values()):
            ...
            legal.normalize()
            temp = []
            for counter in range(0, self.numParticles):
                temp.append(util.sample(legal))
            self.particles = temp
