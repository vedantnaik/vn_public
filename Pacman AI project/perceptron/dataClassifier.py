...
def enhancedFeatureExtractorDigit(datum):
    ...
    We include the number of whitespace regions into the features.
    Digits 1,2,3,5,7 have a single whitespace region
    Digits 6,9 have two whitespace regions
    Digit 8 has 3 whitespace regions
    Digit 4 may have 1 or 2, depending on writer

    We start from any pixel in the datum,
    if the pixel is not lit, nor has one in the datum:
        travel recursively in all four directions and lit up all the pixels
        in the litMatrix (used in noOfWhileSpaces function)
    
    Every time we do this process for a pixel, we update counter.

    EXAMPLE:
    - For cases with 1 region, the first pixel lits up every pixel and so the 
    counter is updated only once.
    - For cases with 2 regions (eg. 6), the pixels inside the 'o' of 6 are not 
    reachable using the updateLitMatrix function

    The features are updated accordingly
    ...
    # We have three possiblities for region of whitespaces => 1,2,3
    features[1] = 0
    features[2] = 0
    features[3] = 0
    features[noOfWhiteSpaces(datum)] = 1
    return features

def noOfWhiteSpaces (datum):
    """
    Given a datum, identify the number of whitespaces region
    """
    counter = 0

    # maintian record of visited pixels and lit them once seen
    litMatrix = []
    for i in range(DIGIT_DATUM_WIDTH):
        temp = []
        for j in range(DIGIT_DATUM_HEIGHT):
            temp.append(0)
        litMatrix.append(temp)
    # initially all set to 0; since no pixel is visited

    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            # for every pixel in datum, cover the connected pixels to identify its span
            if(notLitNorInDatum(x,y,datum,litMatrix)):
                litMatrix = updateLitMatrix(datum, x, y, litMatrix)
                counter += 1
    return counter
    
def notLitNorInDatum(x,y,datum,litMatrix):
    return (datum.getPixel(x,y) == 0 and litMatrix[x][y] == 0)

def updateLitMatrix(datum,x,y,litMatrix):
    """
    Given a pixel, check if the pixels around are lit or is there a white space
    """
    litMatrix[x][y] = 1   # to avoid going in infinite loop, set given pixel as lit
  
    # check left
    if((x != 0) and (notLitNorInDatum(x-1, y, datum, litMatrix))):
        litMatrix = updateLitMatrix(datum, x-1, y, litMatrix)

    # check up
    if((y != 0) and (notLitNorInDatum(x, y-1, datum, litMatrix))):
        litMatrix = updateLitMatrix(datum, x, y-1, litMatrix)
       
    # check right
    if((x !=  DIGIT_DATUM_WIDTH - 1) and (notLitNorInDatum(x+1, y, datum, litMatrix))):
        litMatrix = updateLitMatrix(datum, x+1, y, litMatrix)
       
    # check down
    if((y !=  DIGIT_DATUM_HEIGHT - 1) and (notLitNorInDatum(x, y+1, datum, litMatrix))):
        litMatrix = updateLitMatrix(datum, x, y+1, litMatrix)
...
def enhancedPacmanFeatures(state, action):
    ...
    for food in foodList:
        minFoodDist = min(minFoodDist, util.manhattanDistance(pacmanPos, (food[0],food[1])))
    if foodList == []:
        features[1] = 0
    else:
        features[1] = minFoodDist

    # closest ghost distance
    minGhostDist = 9999999
    for ghost in nextState.getGhostStates():
        minGhostDist = min(minGhostDist, util.manhattanDistance(ghost.getPosition(), pacmanPos))
    features[2] = minGhostDist
